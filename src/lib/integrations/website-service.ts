...
async function persistDocumentToWorker(doc: Document): Promise<void> {
  try {
    ...
    if (!resp.ok) {
      ...
      console.warn('[website-service] Persist to /api/documents failed', resp.status, resp.statusText)
    }
  } catch (err) {
     
    console.warn('[website-service] Persist to /api/documents error', err)
  }
}
